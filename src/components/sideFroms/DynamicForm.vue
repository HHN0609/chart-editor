...
export default defineComponent ({
    ...
    setup (props, {emit}) {
        ...
        function generateVnodes (configTree) {
            let vNodes = Array.prototype.map.call(configTree, (item) => {
                if(item.type === "folder"){
                    let children = [];
                    if(item.content && item.content.length){
                        children =  generateVnodes(item.content);
                    }
                    ...
                    return h(Inputs, { label: item.label, dataIndex: item.dataIndex, type: item.type, value: "" });
                } else if (item.type === "position") {
                    return h(Position, { label: item.label, dataIndex: item.dataIndex, value:"top" });
            ...
            return vNodes ? vNodes : "";
        };
        const vNode = generateVnodes(props.configTree);
        console.log(vNode);
        return () => vNode;
    }
})
